...
public class InRechnungBrowserDialog extends BrowserDialog<Rechnung> {

	private static final long serialVersionUID = 1L;
	RechnungDialogComponents components = null;
 public InRechnungBrowserDialog(Rechnung argRechnung) 
     { 
			...
			public void actionPerformed(ActionEvent e) {
				if(components.bindTo(dataObject))
				{
					if(dataObject.getId()==null)
						Businesslogic.getInstance().addInRechnung(dataObject);
					else
						Businesslogic.getInstance().updateInRechnung(dataObject);
					endDialog();
				}
				return;
